...
// const slug = 'regiao-de-ribeirao-preto/regiao-de-ribeirao-preto/ribeirao-preto';
// const tiposArray = [
// 	'terrenos',
// 	// 'venda'
// 	];

// Ouvindo as mensagens e direcionando-as
/*
		chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
			if(message === 'background')
				console.log('Eu já escuto os seus sinais');
		});
...
tiposArray.forEach(tipo => {
	for(let pagina = 1; pagina <= 100; pagina++){
		fetch(`${urlBase}/imoveis/${tipo}/estado-sp/${slug}?o=${pagina}`,{
			method: 'GET',
			mode: 'no-cors',
...
/*
fetch('https://am.olx.com.br/regiao-de-manaus/imoveis/casa-parintins-am-1121166421?lis=listing_1001')
.then(response => response.text())
.then(response => console.log(response))

*/

// const regioesArray = ['am', 'ba', 'ce', 'mg', 'ms', 'pa', 'pb', 'pr', 'rj', 'rn', 'rs', 'sc', 'sp'];

// regioesArray.forEach(uf => {
// 	fetch(`http://localhost:3000/${uf}`)
// 	.then(response => response.text())
// 	.then(response => {
